...
\par
Chen and Guestrin \citep{chenXGBoostScalableTree2016} further illustrate, using Taylor expansions how, for a fixed structure $q(x)$ the the optimal weight $\omega^*_j$ for a leaf $j$ can be derived. Furthermore they show how to successfully enumarate tree structures using left $I_L$ and right $I_R$ instance sets of nodes and letting $I = I_L \union I_R$. The loss reduction after the split is given by the function:
