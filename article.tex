...
\begin{table}[] 
 ...
 \begin{tabular}{@{}ccccl@{}}
  ...
  \end{tabular}
\end{table}
% FIXME: Add supplementary tables of all classes and another table of the summary statistics of continuous variables. Refer to this table to help with the paragraphs about one-hot encoding. A quick definition of one-hot encoding will also help!
...
XGBoosted (eXtreme Gradient Boosting) trees, described in more detail below,  were created using the XGBoost library \citep{chenXGBoostScalableTree2016} in the Python Programming language \citep{g.vanrossumPythonTutorialTechnical1995}. XGBoosted trees are a boosted tree ensemble method that can be used to classify classes, or predict continuous response variables. They were chosen for this analysis as the data contained a mixture of class and continuous variables. Moreover, XGBoosted trees are unaffected by multicollinearity, and offer high predictive performance for a wide variety of purposes \citep{chenXGBoostScalableTree2016}.
\par
% FIXME: Make it clearer why you did this "Because the purpose of the analysis was to identify relationships between variables and since there was no single outcome variable of interest an XGBoosted tree was created for each variable using the other variables as predictors
% FIXME: forward reference here "XGBoosted trees, described in more detail below...
% FIXME: I found this a bit confusing as a 'new reader'. How about describing this process as a set of analyses:
% Step 1: Separate analyses of operating cost and revenue: XGBoosted models were constructed for each of these variables, with the variable as the response and all other variables (with the exception of operating cost and revenue) as predictors.
% Step 2: For each of the operating cost and revenue models, an analysis of the inter-relationships between the ten most important predictors based on the XGBoosted model.
% Set 3: Analysis of profit (difference between...): etc
% Then say that the results of the analyses in Set 3 were not considered further due to how crap they were.
An XGBoosted tree was created for each variable to show how they interacted. Each tree included all variables except the economic variables (operating cost and revenue from grape sales), which were only included within their own trees as response variables. Separately profit (the difference between revenue and operational costs) was looked at in prior analyses (see appendix) but the results were not included due to low average loss values and model stability. This meant that every variable would have a measure of its importance to other variables (see Section \ref{sec:importance}), which was used to show the highly interrelated nature of variables within vineyards. The interaction between variables was illustrated using Sankey and Chord diagrams; with variable importance measures being used to show the strength of connection between the respective predictor variable and the response (see section \ref{sec:importance}).
\par
% FIXME: We need a digestable description of XGBoost for readers of this article.
\subsection{Variable Importance}\label{sec:importance}
% FIXME: this section can be tightende/shortened a lot - moving some of the material into the supplementary section
...
% FIXME: make sure one-hot-encoding is well defined and rephrase this so it makes more sense (possibly define it earlier as suggested in the data section.)
are given an importance score for each individual class; for example, each specific region will have its own importance score.
\par
...
For each model the data was split into training data, which constituted 80\% of the original data. The remaining 20\% was used in testing and validation. Categorical data was stratified to conserve the same proportion of class occurrences between training, testing and validation data. For continuous variables 20\% was used as testing data and the models were validated using 10 repetitions of the sampling process (10-fold cross validation). $R^2$ scores were used to determine the best regression models during validation. $R^2$ was used instead of RMSE to avoid the issue of comparing models of different units to each other when considering how well each model extrapolated to further data. For binary and multiclass variables, validation was summarised through the accuracy, the proportion of true negatives and positives.
\par
